...
import styles from './tuesday.module.scss';
...
import {
  ...
  selectTuesday,
  setItemsTuesday
} from "../../redux/slices/tuesdaySlice";
import {
  item,
  Status,
...
export function Tuesday() {
  ...
  const { items, status }= useSelector(selectTuesday);
  const [isTask, setIsTask] = useState(items);
  const [value, setValue] = useState('');
  async function fetchTuesdayTasks(){
    dispatch(fetchTuesdayTask(
    ))
  }
