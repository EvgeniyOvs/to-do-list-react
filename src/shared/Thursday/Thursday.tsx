...
import styles from './thursday.module.scss';
...
import {
  ...
  selectThursday,
  setItemsThursday
} from "../../redux/slices/thursdaySlice";
import {
  item,
  Status,
...
export function Thursday() {
  ...
  const { items, status }= useSelector(selectThursday);
  const [isTask, setIsTask] = useState(items);
  const [value, setValue] = useState('');
  async function fetchThursdayTasks(){
    dispatch(fetchThursdayTask(
    ))
  }
  ...
  const onClickAdd = async  () =>{
    ...
    try {
      ...
      const { data } = await axios.post('http://localhost:3002/thursday',
          {id:item.id,
            title:item.title,
            day: item.day,
