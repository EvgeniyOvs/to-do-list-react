...
import {
  ...
  selectSaturday,
  setItemsSaturday
} from "../../redux/slices/saturdaySlice";
import {
  item,
  Status,
...
export function Saturday() {
  ...
  const { items, status }= useSelector(selectSaturday);
  const [isTask, setIsTask] = useState(items);
  const [value, setValue] = useState('');
  async function fetchSaturdayTasks(){
    dispatch(fetchSaturdayTask(
    ))
  }
  ...
  const onClickAdd = async  () =>{
    ...
    try {
      ...
      const { data } = await axios.post('http://localhost:3002/saturday',
          {id:item.id,
            title:item.title,
            day: item.day,
