...
export function Sunday() {
  ...
  const { items, status }= useSelector(selectSunday);
  const [isTask, setIsTask] = useState(items);
  const [value, setValue] = useState('');
  async function fetchSundayTasks(){
    dispatch(fetchSundayTask(
    ))
  }
  ...
  const onClickAdd = async  () =>{
    ...
    try {
      ...
      const { data } = await axios.post('http://localhost:3002/sunday',
          {id:item.id,
            title:item.title,
            day: item.day,
