...
export function Friday() {
  ...
  const { items, status }= useSelector(selectFriday);
  const [isTask, setIsTask] = useState(items);
  const [value, setValue] = useState('');
  async function fetchFridayTasks(){
    dispatch(fetchFridayTask(
    ))
  }
  ...
  const onClickAdd = async  () =>{
    ...
    try {
      ...
      const { data } = await axios.post('http://localhost:3002/friday',
          {id:item.id,
            title:item.title,
            day: item.day,
